...
const complexityFor = (algorithmId: string) => ({ bfs: "TC: O(V + E)", "bidirectional-bfs": "TC: O(V + E)", dfs: "TC: O(V + E)", dijkstra: "TC: O((V + E) log V)", astar: "TC: O((V + E) log V)", greedy: "TC: O((V + E) log V)", beam: "TC: O(BD log B)" }[algorithmId] ?? "TC: —");
