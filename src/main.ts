      ...
      <div class="search-recording-preview" id="recording-preview">
        <section class="search-recording-canvas" aria-label="Portrait recording canvas">
          ...
          <footer class="search-recording-footer">
            ...
            <a class="recording-attribution" href="https://www.openstreetmap.org/copyright" target="_blank" rel="noreferrer">© OpenStreetMap contributors</a>
          </footer>
        </section>
...
const complexityFor = (algorithmId: string) => ({ bfs: "TC: O(V + E)", "bidirectional-bfs": "TC: O(V + E)", dfs: "TC: O(V + E)", iddfs: "TC: O(b^d)", dijkstra: "TC: O((V + E) log V)", astar: "TC: O((V + E) log V)", greedy: "TC: O((V + E) log V)", beam: "TC: O(V + BD log B)" }[algorithmId] ?? "TC: —");
...
const applyEvent = (event: SearchEvent, audible = true) => { if (state.activeId) changedCellIds.add(state.activeId); state.activeId = undefined; if (audible || event.type === "path" || event.type === "miss") sound.playEvent(event, state.currentEventIndex / Math.max(1, state.events.length - 1)); if (event.type === "frontier") { const visited = event.side === "target" ? state.targetVisitedIds : state.visitedIds, frontier = event.side === "target" ? state.targetFrontierIds : state.frontierIds; if (!visited.has(event.id) && !frontier.has(event.id)) { frontier.add(event.id); changedCellIds.add(event.id); state.frontierCount += 1; } } else if (event.type === "visit") { const visited = event.side === "target" ? state.targetVisitedIds : state.visitedIds, frontier = event.side === "target" ? state.targetFrontierIds : state.frontierIds; state.activeId = event.id; changedCellIds.add(event.id); frontier.delete(event.id); if (!visited.has(event.id)) { visited.add(event.id); state.visitedCount += 1; } } else if (event.type === "iteration") { [...state.visitedIds, ...state.frontierIds, ...state.targetVisitedIds, ...state.targetFrontierIds, ...state.pathIds].forEach((id) => changedCellIds.add(id)); state.visitedIds.clear(); state.frontierIds.clear(); state.targetVisitedIds.clear(); state.targetFrontierIds.clear(); state.pathIds.clear(); resetSearchOverlay = true; } else if (event.type === "path") { state.pathIds = new Set(event.ids); event.ids.forEach((id) => changedCellIds.add(id)); state.pathLength = event.ids.length; } else if (event.type === "miss") { state.missed = true; state.pathLength = 0; } };
