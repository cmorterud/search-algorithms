...
const renderCurrentState = () => { render(elements, state, selectedAlgorithm().label); elements.startButton.disabled = state.isRunning || preparingSearch; const complexity = optionalElement<HTMLElement>("#recording-complexity"); if (complexity) complexity.textContent = complexityFor(elements.algorithmSelect.value); };
