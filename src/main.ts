...
const finishRun = () => { sound.stopHum(); state.isRunning = false; state.isPaused = false; state.activeId = undefined; renderCurrentState(); };
const playNextEvent = () => { clearPlaybackTimer(); if (!state.isRunning || state.isPaused) return renderCurrentState(); const speed = Number(elements.speedSlider.value) / 100; const batch = Math.max(1, Math.round(1 + speed ** 1.6 * 220)); for (let i = 0; i < batch; i += 1) { const event = state.events[state.currentEventIndex]; if (!event) return finishRun(); applyEvent(event, i === batch - 1); state.currentEventIndex += 1; } renderCurrentState(); playbackTimer = window.requestAnimationFrame(playNextEvent); };
...
const resetRun = () => { clearPlaybackTimer(); sound.stopHum(); state = createState(state.originalGrid); renderCurrentState(); };
const randomize = () => { clearPlaybackTimer(); sound.stopHum(); state = createState(isRecording ? (loadedCity ? createGridFromCityGraph(loadedCity) : createCityGrid()) : createMazeGrid(Number(elements.densitySlider.value))); renderCurrentState(); };
const clearGrid = () => { clearPlaybackTimer(); sound.stopHum(); state = createState(createOpenGrid()); renderCurrentState(); };
