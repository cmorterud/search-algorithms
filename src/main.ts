...
const playNextEvent = () => { clearPlaybackTimer(); if (!state.isRunning || state.isPaused) return renderCurrentState(); const speed = Number(elements.speedSlider.value) / 100; const batch = Math.max(1, Math.round(1 + speed * 72)); for (let i = 0; i < batch; i += 1) { const event = state.events[state.currentEventIndex]; if (!event) return finishRun(); applyEvent(event, i === batch - 1); state.currentEventIndex += 1; } renderCurrentState(); playbackTimer = window.requestAnimationFrame(playNextEvent); };
