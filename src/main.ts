...
const applyEvent = (event: SearchEvent, audible = true) => { if (state.activeId) changedCellIds.add(state.activeId); state.activeId = undefined; if (audible) sound.playEvent(event, state.currentEventIndex / Math.max(1, state.events.length - 1)); if (event.type === "frontier" && !state.visitedIds.has(event.id) && !state.frontierIds.has(event.id)) { state.frontierIds.add(event.id); changedCellIds.add(event.id); state.frontierCount += 1; } else if (event.type === "visit") { state.activeId = event.id; changedCellIds.add(event.id); state.frontierIds.delete(event.id); if (!state.visitedIds.has(event.id)) { state.visitedIds.add(event.id); state.visitedCount += 1; } } else if (event.type === "path") { state.pathIds = new Set(event.ids); event.ids.forEach((id) => changedCellIds.add(id)); state.pathLength = event.ids.length; } else if (event.type === "miss") { state.missed = true; state.pathLength = 0; } };
